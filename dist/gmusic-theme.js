  ...
  _createClass(GMusicTheme, [{
    ...
    */
    ...
    value: function _substituteColors(styleString) {
      ...
      return styleString.replace(/<<BACK_PRIMARY>>/g, this.BACK_PRIMARY).replace(/<<BACK_SECONDARY>>/g, this.BACK_SECONDARY).replace(/<<BACK_HIGHLIGHT>>/g, this.BACK_HIGHLIGHT).replace(/<<FORE_PRIMARY>>/g, this.FORE_PRIMARY).replace(/<<FORE_SECONDARY>>/g, this.FORE_SECONDARY).replace(/<<INVERSE>>(.+?)<<\/INVERSE>>/g, function (s, colorString) {
        return _this2._inverse(colorString);
      }).replace(/<<DARKEN ([01]+.?[0-9]*)>>(.+?)<<\/DARKEN>>/g, function (s, n, colorString) {
        return _this2._darken(colorString, parseFloat(n));
