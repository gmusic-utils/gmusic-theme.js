...
window.GMusicTheme = (function () {
  ...
  _createClass(GMusicTheme, [{
    ...
    key: 'updateBackPrimary',
    value: function updateBackPrimary(color) {
      this.BACK_PRIMARY = color;
      this.redrawTheme();
    }
  }, {
    key: 'updateBackSecondary',
    value: function updateBackSecondary(color) {
      this.BACK_SECONDARY = color;
      this.redrawTheme();
    }
  }, {
    key: 'updateBackHighlight',
    value: function updateBackHighlight(color) {
      this.BACK_HIGHLIGHT = color;
      this.redrawTheme();
    }
  }, {
    key: 'updateForePrimary',
    value: function updateForePrimary(color) {
      this.FORE_PRIMARY = color;
      this.redrawTheme();
    }
  }, {
    key: 'updateForeSecondary',
    value: function updateForeSecondary(color) {
    ...
    key: '_addStyle',
    value: function _addStyle(selector, styles, importantToggle) {
      var _this = this;

      var string = 'body.custom ' + selector.replace(/, /g, ', body.custom ') + ' {';
      styles.forEach(function (style) {
        string += _this._substituteColors(style) + (importantToggle ? '' : ' !important;');
      });
      string += '}';
    ...
    key: '_fire',
    value: function _fire(event, details) {
      this.events[event] = this.events[event] || [];
      ...
      setInterval(function () {
        ...
        if (logo) {
          ...
          if (localStorage.getItem('custom-theme') === 'true') {
            if (logo.nodeName === 'IMG' || logo.id === 'normalSVGIcon' || logo.getAttribute('current-custom') !== _this2.FORE_SECONDARY) {
              parent.removeChild(logo);
              tmpSVG = new DOMParser().parseFromString(customSVG, 'text/xml').firstChild;
              tmpSVG.setAttribute('current-custom', _this2.FORE_SECONDARY);
